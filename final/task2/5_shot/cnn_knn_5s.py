...
parser.add_argument('-p', '--prediction_file', default='./prediction_5s.csv', type=str)
parser.add_argument('-s', '--save_model_dir', default='./saved_model/', type=str)
parser.add_argument('-lcnn', '--load_model_file', default='../model/cnn_model_5s.h5', type=str)
parser.add_argument('-lknn', '--knn_clf_file', default='../model/knn_clf_5s.pickle', type=str)
...
parser.add_argument('-bs', '--best_seed', default=23, type=int)
...
def CNN_model(extract=False):
    ...
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv2', kernel_regularizer=l2(weight_decay))(img_input)
    ...
    model.summary()
    if extract:
        extract_model = Model(inputs=model.input, outputs=model.get_layer('fc1').output)
...
def main():
    if args.action == 'train_cnn':
        ...
        pickle.dump(best_knn_clf, open(os.path.join(args.save_model_dir, 'knn_clf_5s.pickle'), 'wb'))
