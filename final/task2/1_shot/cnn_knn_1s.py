...
parser.add_argument('-p', '--prediction_file', default='./prediction_1s.csv', type=str)
parser.add_argument('-s', '--save_model_dir', default='./saved_model/', type=str)
parser.add_argument('-lcnn', '--load_model_file', default='../model/cnn_model_1s.h5', type=str)
parser.add_argument('-lknn', '--knn_clf_file', default='../model/knn_clf_1s.pickle', type=str)
...
parser.add_argument('-bs', '--best_seed', default=15, type=int)
...
def CNN_model(extract=False):
    ...
    model.summary()
    if extract:
        extract_model = Model(inputs=model.input, outputs=model.get_layer('fc2').output)
...
def main():
    if args.action == 'train_cnn':
        ...
        for seed in range(30):
            ...
            knn_clf.fit(cnn_feature, y_novel_train)
        ...
        pickle.dump(best_knn_clf, open(os.path.join(args.save_model_dir, 'knn_clf_1s.pickle'), 'wb'))
